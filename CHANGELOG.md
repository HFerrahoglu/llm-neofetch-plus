...
### Added
- **"Right now" verdicts in `can-run`**: alongside the capacity verdict
  (hardware totals), each quantization now gets a second verdict computed
  from memory that is actually free at this moment — available RAM and
  unused VRAM (total minus used, per-GPU). The header shows free amounts
  next to totals. Addresses long-standing feedback that recommendations
  ignored memory already tied up by other processes.
